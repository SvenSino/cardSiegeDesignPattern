...
den Platzierungsmarker mit Reichweitenkreis. Die Methode \texttt{tryPlace\-Pending\-Tower(x, y)}
...
Die Methode \texttt{upgradeTower(tower)} umhüllt den Turm mit einem
...
Die Methode \texttt{update\-Range\-Observers()} berechnet pro Turm per Quadratdistanzvergleich,
...
\texttt{slowFactor} für eine begrenzte Dauer. Die Methode \texttt{isInRange(tower)} prüft
...
Die Methode \texttt{cleanupEntities()} entfernt tote Gegner (+1 Gold,
...
Die Methode \texttt{Deck.draw()} liefert die oberste Karte; ist der Stapel leer, füllt
\texttt{refill()} ihn über Factory und Prototype-Klonen neu auf und mischt ihn
anschließend. Die Klasse \texttt{Hand} verwaltet die Handkarten als Liste;
\texttt{addToFront()} wird beim Zurücklegen einer abgelehnten Karte genutzt.

Die Methode \texttt{ensure\-First\-Hand\-Has\-Tower()} garantiert, dass die allererste Hand des
...
Die Klasse \texttt{Tile} repräsentiert eine einzelne Kachel mit Gitterkoordinaten und
einem booleschen \texttt{path}-Flag. Die Klasse \texttt{Board} hält die Kachelliste,
...
Gitterkoordinaten landen in einem \texttt{LinkedHashSet}, das Duplikate
entfernt und die Reihenfolge erhält. Jede Koordinate wird als
...
Die Klasse \texttt{Projectile} ist ein reines Darstellungsobjekt ohne Einfluss auf die
Spiellogik. Es speichert Start- und Zielposition sowie eine
\texttt{ttl}-Zeit von 0,15~Sekunden. Die Methode \texttt{getAlpha()} liefert einen linear
abfallenden Transparenzwert für weiches Ausblenden; \texttt{isExpired()}
...
Darstellung. Die Methode \texttt{render()} delegiert zunächst \texttt{update(delta)} an
...
Die Methode \texttt{drawPlacementPreview()} ist aktiv, solange ein \texttt{PendingTower}
